...
#######################################################
gen_RRs <- function(summary, indices, round = 2) {
  summary <- as.matrix(summary[, indices])
  ref <- paste0("Ref RR = 1")
  storage <- c()
  for (i in 1:dim(summary)[2]) {
    temp <- paste0(round(summary[2, i], round), " (", round(summary[1, i], round), " - ", round(summary[3, i], round), ")")
...
total_empirical_summaries <- function(data, variable, income_strata) {
  ...
  summary <- temp %>%
    ...
    summarise(mean = mean(tot_all, na.rm = TRUE),
              lower = quantile(tot_all, probs = 0.25, na.rm = TRUE),
              upper = quantile(tot_all, probs = 0.75, na.rm = TRUE),
              n = n())
    # summarise(mean = mean(tot_all, na.rm = TRUE),
...
physical_generate_forestplot_data <- function(data, model, income_strata) {
  ...
  if (model == "Univariate") {
    ...
    method <- if (income_strata == "HIC") NA else readRDS(file = paste0("Outputs/Univariate/", files[pmatch(paste0("physical", "_", income_strata, "_method"), files)]))
    ...
    method <- if (income_strata == "HIC") NA else readRDS(file = paste0("Outputs/Multivariate/", files[pmatch(paste0("physical", "_", income_strata, "_age3cat_gender_method"), files)]))
  ...
  # Extract Model Coefficients
  age_coef <- physical_summarise_coefs(age)[, c("age3cat2", "age3cat3")]
  gender_coef <- physical_summarise_coefs(gender)[, "gender", drop = FALSE] 
  weekday_coef <- physical_summarise_coefs(weekday)[, "weekday1", drop = FALSE]
  hh_coef <- physical_summarise_coefs(hh)[, c("hh_size2", "hh_size3", "hh_size4", "hh_size5", "hh_size6P")]
  method_coef <- if (income_strata == "HIC") data.frame(rep(NA, 3)) else physical_summarise_coefs(method)[, "methodInterview", drop = FALSE]
  student_coef <- physical_summarise_coefs(student)[, "student1", drop = FALSE]
  employment_coef <- physical_summarise_coefs(employment)[, "employment1", drop = FALSE]
...
duration_generate_forestplot_data <- function(data, model, income_strata) {
  ...
  # Extract Model Coefficients
  age_coef <- duration_summarise_coefs(age)[, c("age3cat2", "age3cat3")]
  gender_coef <- duration_summarise_coefs(gender)[, "gender", drop = FALSE] 
  weekday_coef <- duration_summarise_coefs(weekday)[, "weekday1", drop = FALSE]
  hh_coef <- duration_summarise_coefs(hh)[, c("hh_size2", "hh_size3", "hh_size4", "hh_size5", "hh_size6P")]
  method_coef <- duration_summarise_coefs(method)[, "methodInterview", drop = FALSE]
  student_coef <- duration_summarise_coefs(student)[, "student1", drop = FALSE]
  employment_coef <- duration_summarise_coefs(employment)[, "employment1", drop = FALSE]
